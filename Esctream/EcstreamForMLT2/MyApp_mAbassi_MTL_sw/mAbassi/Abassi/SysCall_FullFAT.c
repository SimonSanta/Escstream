/* ------------------------------------------------------------------------------------------------ */
/* FILE :		SysCall_FullFAT.c																	*/
/*																									*/
/* CONTENTS :																						*/
/*				System call layer with underlying FullFAT											*/
/*																									*/
/*																									*/
...
/* ------------------------------------------------------------------------------------------------ */
/* Limitations / non-standard things due to FullFAT													*/
/*																									*/
/* FullFAT formatting seems to have issues. Therefore mkfs() is always returning a failure.			*/
/*																									*/
/* FullFAT does not have an API to sync (or flush) a file.  It only offer a per drive sync.			*/
/* Whenever a file sync should occur, the whole drive is synced.  This could provoke a major 		*/
/* slowdown when using write() if the file was opened with the O_SYNC flags as the whole drive		*/
/* will be synced after each call to write() for an O_SYNC opened file.								*/
/*																									*/
/* ------------------------------------------------------------------------------------------------	*/
...
#ifndef FF_BLKDEV_USES_SEM
  #error "FF_BLKDEV_USES_SEM must be defined for safe operation"
#endif

#ifndef FF_FINDAPI_ALLOW_WILDCARDS
  #error "FF_FINDAPI_ALLOW_WILDCARDS muts be defined for proper operation of SysCall"
#endif

#ifdef FF_OPTIMISE_UNALIGNED_ACCESS
  #error "Don't define FF_OPTIMISE_UNALIGNED_ACCESS as issues were found with it"
#endif
...
/* ------------------------------------------------------------------------------------------------ */
  ...
  #define SYS_CALL_MULTI_FULLFAT	0				/* By default, single file system stack			*/
#endif

#if ((SYS_CALL_MULTI_FULLFAT) != 0)					/* When more than 1 FS stack is used			*/
  #define P_NM _FullFAT								/* Each ones of the function in here are		*/
...
typedef struct {
	FF_IOMAN *Fsys;									/* FullFAT file system descriptor				*/
...
typedef struct {									/* Our own file descriptors						*/
	...
	FF_FILE *Fdesc;									/* FullFAT file descriptor						*/
...
typedef struct SysDir {								/* Our own directory descriptors				*/
	...
	FF_DIRENT Ddesc;								/* FullFAT directory descriptor					*/
	FF_DIRENT Dfirst;								/* First entry in the open directory (for seek)	*/
...
#define SX_IARFLUSH_(P2,x)						IARflush##P2(x)
  ...
  #define REENT_ERRNO_							(errno)
	...
	int fstat_FullFAT(int fd, struct stat *pstat);
	int fstatfs_FullFAT(int fs, struct statfs *buf);
	int stat_FullFAT(const char *path, struct stat *pstat);
	int unlink_FullFAT(const char *path);
  #endif
  ...
  #if ((SYS_CALL_MULTI_FULLFAT) != 0)
	int _fstat_r_FullFAT(struct _reent *reent, int fd, struct stat *pstat);
	int _fstatfs_r_FullFAT(struct _reent *reent, int fs, struct statfs *buf);
	int _stat_r_FullFAT(struct _reent *reent, const char *path, struct stat *pstat);
	int _unlink_r_FullFAT(struct _reent *reent, const char *path);
  #endif
...
#if ((SYS_CALL_MULTI_FULLFAT) != 0)
  DIR_t         *opendir_FullFAT(const char *path);
  struct dirent *readdir_FullFAT(DIR_t *dirp);
  int            closedir_FullFAT(DIR_t *dirp);
#endif

#define MY_FAT_NAME	"FatFS"

#if ((SYS_CALL_MUTEX) >  0)	
static const char g_DdscName[][16] = {
                                      "Ddsc " MY_FAT_NAME "  0"
                                   #if ((SYS_CALL_N_DIR) > 1)
                                     ,"Ddsc " MY_FAT_NAME "  1"
                                   #endif
...
/* ------------------------------------------------------------------------------------------------ */
/* Local defintions as no Media_FullFAT.h available													*/
...
extern int         media_init_FullFAT     (int Drv);
extern FF_T_SINT32 media_read_FullFAT     (FF_T_UINT8 *buffr, FF_T_UINT32 sector, FF_T_UINT32 count,
                                           void *pParam);
extern int64_t     media_size_FullFAT     (int Drv);
extern int         media_uninit_FullFAT   (int Drv);
extern FF_T_SINT32 media_write_FullFAT    (FF_T_UINT8 *buffr, FF_T_UINT32 sector, FF_T_UINT32 count,
                                           void *pParam);
...
int      FatFlags;									/* Remapped flags to the FullFAT tokens			*/
int      fd;										/* File descriptor value to return				*/
FF_ERROR Fres;										/* Result of FullFAT calls						*/
int      Omode;										/* Open mode									*/
char     Rpath[SYS_CALL_MAX_PATH+1];				/* Real path using logical drive #				*/
struct stat pstat;									/* Used to know if existing file is R/O or R/W	*/
	...
	if (Access == O_WRONLY) {						/* These 3 O_??? are not ORed bit definitions	*/
		FatFlags = FF_MODE_WRITE;					/* Need to do one on one comparisons			*/
		Omode    = 0333;
	}
	else if (Access == O_RDWR) {
		FatFlags = FF_MODE_READ | FF_MODE_WRITE;
		Omode    = 0777;
	}
	else {											/* O_RDONLY is the else as is a safe opening	*/
		FatFlags = FF_MODE_READ;					/* if caller forgot to set the desired access	*/
		Omode    = 0555;
	}

	FatFlags |= (0 != (flags & O_CREAT))
	          ? FF_MODE_CREATE
	          : 0;
	FatFlags |= (0 != (flags & O_APPEND))
	          ? FF_MODE_APPEND
	          : 0;
	FatFlags |= (0 != (flags & O_TRUNC))
	          ? FF_MODE_TRUNCATE
	          : 0;
	...
	if (Err == 0) {									/* With O_CREAT|O_EXCL, the file to open can't	*/
		if (((O_EXCL|O_CREAT) == (flags & (O_EXCL|O_CREAT)))	/* exist. FullFAT does not have a	*/
	  #if ((SYS_CALL_DEV_USED) != 0)				/* flag when opening to refuse creating an		*/
		&& (Drv >= 0)								/* existing file								*/
	  #endif
		&&  (0 == SC_STAT_(P_NM, reent, Rpath, &pstat)))	 {
			Err = EEXIST;							
		}
		...
		if (Err == 0) {
			...
			{										/* Opening a regular file						*/
				...
				if (Err == 0) {
					g_SysFile[fd].Fdesc = FF_Open(g_SysMnt[Drv].Fsys, &Rpath[2], FatFlags, &Fres);
					if (Fres != FF_ERR_NONE) {
						Err = EACCES;				/* Remap any error to an access error			*/
					}
				}

				if (Err == 0) {						/* Successfully opened, fill file descriptor	*/
					strcpy(&g_SysFile[fd].Fname[0], &Rpath[0]);
				  #if ((SYS_CALL_DEV_USED) != 0)
...
{
int  Drv;											/* Drive # to flush the I/O manager cache		*/
int  Err;											/* Error tracking								*/
	...
	||  (fd >= (int)(sizeof(g_SysFile)/sizeof(g_SysFile[0])))) {
		...
		&&  (g_SysFile[fd].DevType == 0)
			...
			if (FF_ERR_NONE != FF_Close(g_SysFile[fd].Fdesc)) {
				Err = EACCES;
			}

			Drv = g_SysFile[fd].Fname[0] - '0';
			if (FF_ERR_NONE != FF_FlushCache(g_SysMnt[Drv].Fsys)) {
				Err = EACCES;
			}
  ...
  int DevType = 0;
	...
	||  (fd >= (int)(sizeof(g_SysFile)/sizeof(g_SysFile[0])))) {
		...
		if (Err == 0) {								/* File open and read operation authorized		*/
		  ...
		  #endif
			{
				Ret = FF_Read(g_SysFile[fd].Fdesc, 1, size, vbuf);
				if (Ret < 0) {
					Err = EACCES;
				}
			}
...
/*		stdin / stdout / stderr are properly handled through the UART								*/
/*																									*/
/*		FullFAT does not provide a "sync" API to flush the file, so if sync was requested at		*/
/*		opening, it is ignored																		*/
/*																									*/
/* IMPLEMENTATION:																					*/
/* - Check validity of the file descriptor (fd), if not then error / exit							*/
/* - If stdin / stdout /stderr																		*/
  ...
  int DevType = 0;
	...
	||  (fd >= (int)(sizeof(g_SysFile)/sizeof(g_SysFile[0])))) {
		...
		if (Err == 0) {
		  ...
		  #endif
			{
				Ret = FF_Write(g_SysFile[fd].Fdesc, len, 1, (FF_T_UINT8 *)vbuf);
				if (Ret < 0) {
					Err = EACCES;
				}
				else if (g_SysFile[fd].Sync != 0) {	/* If open with sync, flush after each write	*/
					Drv = g_SysFile[fd].Fname[0] - '0';
					FF_FlushCache(g_SysMnt[Drv].Fsys);
				}
			}
		}
...
int   Origin;										/* Remapped "whence" to the FullFAT argument	*/
	...
	||  (fd >= (int)(sizeof(g_SysFile)/sizeof(g_SysFile[0])))) {
		...
		if (Err == 0) {
			...
			if (FF_ERR_NONE != FF_Seek(g_SysFile[fd].Fdesc, offset, Origin)) {
				Err = EACCES;
			}
			OffRes = (off_t)g_SysFile[fd].Fdesc->FilePointer;
		}
...
struct stat Pstat;									/* To retrieve access mode of old				*/
	...
	if (Err == 0) {
		if (DrvNew == DrvOld) {						/* Same file system, use FF_Move() directly		*/
			if (FF_ERR_NONE != FF_Move(g_SysMnt[DrvOld].Fsys, &OldPath[2], &NewPath[2])) {
				Err = EACCES;
			}
		}
...
/*																									*/
/* NOTE:																							*/
/*		FullFAT FF_RmDir() removes non-empty directories. As it's standard practice to not allow	*/
/*		the removal of a non-empty directory, the directory is checked to be empty.  Nope, can't	*/
/*		use the FullFAT FF_isDirEmpty() function as it always report empty even when non-empty.		*/
/*																									*/
/* IMPLEMENTATION:																					*/
/*																									*/
/* ------------------------------------------------------------------------------------------------ */
...
int    Err;											/* Error tracking								*/
DIR_t *MyDir;										/* When rmdir, to check if dir is empty			*/
char   Rpath[SYS_CALL_MAX_PATH+1];					/* Real path									*/
struct dirent *Dentry;								/* Directory entry to check if dir is empty		*/
struct stat    pstat;								/* To determine if is a regular file or dir		*/
	...
	if (Err == 0) {									/* Need to know if it's a dir or a regular file	*/
		if (0 != SC_STAT_(P_NM, reent, &Rpath[0], &pstat)) {
			Err = ENOENT;
		}

		if (Err == 0) {
			if (0 != (pstat.st_mode & S_IFDIR)) {	/* When it's a directory, has to use FF_RmDir()	*/
				MyDir = SC_OPENDIR_(P_NM, Rpath);	/* Need to check if the directory is empty		*/
				if (MyDir == (DIR_t *)NULL) {		/* because FullFAT deletes non-empty dir		*/
					Err = ENOENT;
				}									/* A directory is empty when only "." and ".."	*/
				else {								/* are in it									*/
					while ((NULL != (Dentry = SC_READDIR_(P_NM, MyDir)))
					&&     (Err == 0)) {
						if ((0 != strcmp(&Dentry->d_name[0], "."))
						&&  (0 != strcmp(&Dentry->d_name[0], ".."))) {
							Err = ENOTEMPTY;
						}
					}

					if ((Err == 0)
					&&  (FF_ERR_NONE != FF_RmDir(g_SysMnt[Drv].Fsys, &Rpath[2]))) {
						Err = EACCES;
					}
					SC_CLOSEDIR_(P_NM, MyDir);
				}
			}
			else {									/* Regular file, use FF_RmFile()				*/
				if (FF_ERR_NONE != FF_RmFile(g_SysMnt[Drv].Fsys, &Rpath[2])) {
					Err = EACCES;
				}
			}
		}
	...
	if (Err == 0) {
		if (g_SysFile[fd].IsOpen == 0) {			/* Make sure the file desc is valid. In other	*/
			...
			if (fn >= (int)(sizeof(g_SysFile)/sizeof(g_SysFile[0]))) {	/* All entries are used		*/
				...
				g_SysFile[fn].Fdesc = FF_MALLOC(sizeof(*g_SysFile[fn].Fdesc));
				memmove(g_SysFile[fn].Fdesc, g_SysFile[fd].Fdesc, sizeof(*g_SysFile[fn].Fdesc));

				IoMan = g_SysFile[fn].Fdesc->pIoman;
				FF_PendSemaphore(IoMan->pSemaphore);
				g_SysFile[fd].Fdesc->Next = g_SysFile[fn].Fdesc;
				FF_ReleaseSemaphore(IoMan->pSemaphore);

													/* There is #error about this, put this here in	*/
			  #ifdef FF_OPTIMISE_UNALIGNED_ACCESS	/* put this here in case user overrides			*/
				g_SysFile[fn].Fdesc->pBuf = FF_MALLOC(IoMan->BlkSize);
				if (g_SysFile[fn].Fdesc->pBuf == NULL) {
					Err = ENOMEM;
					FF_FREE(g_SysFile[fn].Fdesc);
				}
				memmove(g_SysFile[fn].Fdesc->pBuf, g_SysFile[fn].Fdesc->pBuf, IoMan->BlkSize);
			  #endif
...
char DirName[SYS_CALL_MAX_PATH+1];					/* We use this in case the directory is invalid	*/
	...
	if (Err == 0) {									/* Valid drive, memo new CWD in task descriptor	*/
...
char Rpath[SYS_CALL_MAX_PATH+1];
	...
	if ((Err == 0)									/* Simple: direct use of the FullFAT function	*/
	&&  (FF_ERR_NONE != FF_MkDir(g_SysMnt[Drv].Fsys, &Rpath[2]))) {
		Err = EACCES;
	}
...
/* ------------------------------------------------------------------------------------------------ */
static void FillStat(struct stat *pstat, FF_DIRENT  *DirInfo, int IsMnt)
{
  ...
  struct tm Time;
	...
	if (IsMnt != 0) {
		memset(&pstat->st_atime, 0, sizeof(pstat->st_atime));
		memset(&pstat->st_ctime, 0, sizeof(pstat->st_ctime));
		memset(&pstat->st_mtime, 0, sizeof(pstat->st_mtime));
	  ...
	  #ifdef FF_TIME_SUPPORT
		Time.tm_year = DirInfo->AccessedTime.Year
		             - 1900;
		Time.tm_mon  = DirInfo->AccessedTime.Month;
		Time.tm_mday = DirInfo->AccessedTime.Day;
		Time.tm_hour = DirInfo->AccessedTime.Hour;
		Time.tm_min  = DirInfo->AccessedTime.Minute;
		Time.tm_sec  = DirInfo->AccessedTime.Second;
		pstat->st_atime = mktime(&Time);

		Time.tm_year = DirInfo->CreateTime.Year
	             - 1900;
		Time.tm_mon  = DirInfo->CreateTime.Month;
		Time.tm_mday = DirInfo->CreateTime.Day;
		Time.tm_hour = DirInfo->CreateTime.Hour;
		Time.tm_min  = DirInfo->CreateTime.Minute;
		Time.tm_sec  = DirInfo->CreateTime.Second;
		pstat->st_ctime = mktime(&Time);

		Time.tm_year = DirInfo->ModifiedTime.Year
		             - 1900;
		Time.tm_mon  = DirInfo->ModifiedTime.Month;
		Time.tm_mday = DirInfo->ModifiedTime.Day;
		Time.tm_hour = DirInfo->ModifiedTime.Hour;
		Time.tm_min  = DirInfo->ModifiedTime.Minute;
		Time.tm_sec  = DirInfo->ModifiedTime.Second;
		pstat->st_mtime = mktime(&Time);
	  #else
		memset(&pstat->st_mtime, 0, sizeof(pstat->st_mtime));
	  #endif

		pstat->st_mode  = (S_IRUSR | S_IRGRP | S_IROTH)	/* FullFAT is always readable				*/
		                | (S_IXUSR | S_IXGRP | S_IXOTH)	/* FullFAT is always executable				*/
		                | (((DirInfo->Attrib) & FF_FAT_ATTR_READONLY)
		                  ? 0
		                  : (S_IWUSR | S_IWGRP | S_IWOTH))
		                | (((DirInfo->Attrib) & FF_FAT_ATTR_DIR)
		                  ? S_IFDIR
		                  : S_IFREG);
		pstat->st_size  = (off_t)DirInfo->Filesize;	/* Size of the file								*/
	}
...
int        Ret;										/* Return value									*/
	...
	||  (fd >= (int)(sizeof(g_SysFile)/sizeof(g_SysFile[0])))) {
		...
		if (g_SysFile[fd].IsOpen == 0) {			/* Must be an open file							*/
		  ...
		  #endif
			{
				Drv = (int)(g_SysFile[fd].Fname[0]-'0');

				if (FF_ERR_NONE != FF_GetEntry(g_SysMnt[Drv].Fsys, g_SysFile[fd].Fdesc->DirEntry,
			                               g_SysFile[fd].Fdesc->DirCluster, &DirInfo)) {
					REENT_ERRNO = EACCES;
				}
				else {
					FillStat(pstat, &DirInfo, 0);
					Ret = 0;
				}
			}
		}
...
/*																									*/
/* IMPLEMENTATION:																					*/
/*		Uses _fstat_r() as easier to get stats from an already open file							*/
/*																									*/
/* ------------------------------------------------------------------------------------------------ */

int SC_STAT_(P_NM, struct _reent *reent, const char *path, struct stat *pstat)
{
FF_T_UINT32 Dclust;									/* Cluster of the directory						*/
int         dd;										/* Index in g_SysMnt[]							*/
FF_DIRENT   Dentry;									/* Directory entry of the file					*/
int         Dev;									/* When /dev supported, device #				*/
int         Drv;									/* Drive # of the file to chmod()				*/
int         Err;									/* Error tracking								*/
FF_T_UINT32 Fclust;									/* Cluster of the file							*/
char        Fname[SYS_CALL_MAX_PATH+1];				/* File name without the path					*/
FF_ERROR    Fres;									/* Result of FullFAT calls						*/
int         ii;										/* General purpose								*/
int         IsMnt;									/* Is is a mount point full or or partial path	*/
int         Len;									/* Length of path (without the file name		*/
char       *Ptr;									/* Used to find the last '/' in full path		*/
char        Rpath[SYS_CALL_MAX_PATH+1];
	...
	if (Err == 0) {
	  ...
	  #endif
		{
			...
			if (IsMnt == 0) {
				...
				Ptr = strrchr(&Rpath[0], '/');		/* Split the full path in path & file name		*/
				strcpy(&Fname[0], Ptr+1);			/* after last '/' is the file name				*/
				if (Ptr == &Rpath[2]) {				/* When is at the root level, keep '/'			*/
					Ptr++;
				}
				*Ptr = '\0';						/* Terminate the path							*/
				Len  = strlen(Rpath);				/* Length of the full path / no file name		*/

				Dclust = FF_FindDir(g_SysMnt[Drv].Fsys, &Rpath[2], Len-2, &Fres);
				if ((Dclust == 0)					/* Invalid cluster #							*/
				||  (Fres != FF_ERR_NONE)) {		/* Error reported								*/
					Err = EBADF;
				}

				if (Err == 0) {
					Fclust = FF_FindEntryInDir(g_SysMnt[Drv].Fsys, Dclust, &Fname[0], 0,
					                           &Dentry, &Fres);
					if ((Fclust == 0)				/* Invalid cluster #							*/
					||  (Fres != FF_ERR_NONE)) {	/* Error reported								*/
						Err = ENOENT;
					}
				}
...
{
char     DrvNmb[3];									/* Drive # string as required by FullFAT		*/
int      Err;										/* Error tracking								*/
FF_ERROR Ferr;
	...
	if (Err == 0 ) {
		...
		buf->f_bsize  = g_SysMnt[fs].Fsys->BlkSize;
		buf->f_blocks = (int64_t)g_SysMnt[fs].Fsys->pPartition->NumClusters
		              * (int64_t)g_SysMnt[fs].Fsys->pPartition->SectorsPerCluster;
		buf->f_bfree  = (int64_t)FF_GetFreeSize(g_SysMnt[fs].Fsys, &Ferr)
		              / (int64_t)buf->f_bsize;
...
char        Rpath[SYS_CALL_MAX_PATH+1];
	...
	if (Drv < 0) {									/* Invalid drive or /dev (don;t accept chmod)	*/
		...
		*Ptr = '\0';								/* Terminate the path							*/
		...
		if (Err == 0) {
			Fclust = FF_FindEntryInDir(g_SysMnt[Drv].Fsys, Dclust, &Fname[0], 0, &Dentry, &Fres);
			if ((Fclust == 0)						/* Invalid cluster #							*/
			||  (Fres != FF_ERR_NONE)) {			/* Error reported								*/
				Err = ENOENT;
			}
		...
		if (Err == 0) {								/* We're good, got the file directory entry		*/
			if (0 != (mode & (S_IWUSR|S_IWGRP|S_IWOTH))) {
				Dentry.Attrib &= ~(FF_FAT_ATTR_READONLY);
			}
			else {
				Dentry.Attrib |= FF_FAT_ATTR_READONLY;
			}
													/* Put the modified entry back					*/
			Fres = FF_PutEntry(g_SysMnt[Drv].Fsys, Dentry.CurrentItem-1, Dclust, &Dentry);
			if (Fres != FF_ERR_NONE) {
				Err = EACCES;
			}

			FF_FlushCache(g_SysMnt[Drv].Fsys);		/* Make sure it get written to the device		*/
		}
	}
...
/*																									*/
/*      FullFAT mounting APIs do not specify the desired FAT type (FAT12, FAT16, or FAT32).			*/
/*		So as long as the requested format indicated is one for the three (FAT12, FAT16, FAT32),	*/
/*		the format will be accepted and the resulting format is determined by FullFAT				*/
/*																									*/
/* IMPLEMENTATION:																					*/
/*																									*/
/* ------------------------------------------------------------------------------------------------ */
...
char *Ptr;											/* "data" argument is the logical drive #		*/
	...
	if (Err == 0) {
		if (g_SysMnt[Drv].MntPoint[0] != '\0') {	/* Is mounted, can only update					*/
			...
			&&  (0 == media_init_FullFAT(Drv))) {
				Err = ENXIO;
			}

			BlkSize = media_sectsize_FullFAT(Drv);
			if (BlkSize < 512) {
				BlkSize = 512;
			}
			...
			if (Err == 0) {
				g_SysMnt[Drv].Fsys = FF_CreateIOMAN(NULL, 2*BlkSize, BlkSize, NULL);
				if (g_SysMnt[Drv].Fsys == NULL) {
					Err = ENXIO;
				}
			}

			if (Err == 0) {							/* Put drive # as the parameter pointer			*/
				if (FF_ERR_NONE != FF_RegisterBlkDevice(g_SysMnt[Drv].Fsys, BlkSize,
				                                 (FF_WRITE_BLOCKS) media_write_FullFAT,
				                                 (FF_READ_BLOCKS)  media_read_FullFAT,
					                             (void *)(intptr_t)Drv)) {
					FF_DestroyIOMAN(g_SysMnt[Drv].Fsys);
					Err = ENXIO;
				}
			}

			if (Err == 0) {
				if (FF_ERR_NONE != FF_MountPartition(g_SysMnt[Drv].Fsys, 0)) {
					media_init_FullFAT(Drv);		/* Automatic retry needed (mystery!)			*/
					if (FF_ERR_NONE != FF_MountPartition(g_SysMnt[Drv].Fsys, 0)) {
						FF_DestroyIOMAN(g_SysMnt[Drv].Fsys);
						Err = ENXIO;
					}
			...
			if (Err == 0) {							/* Memo the mount point in g_SysMnt				*/
				g_SysMnt[Drv].Flags = flags;
				switch (g_SysMnt[Drv].Fsys->pPartition->Type) {
				case FF_T_FAT12:
...
int  Len;											/* String length								*/
	...
	if (0 != MTXlock(g_GlbMtx, -1)) {				/* Need global protection for that				*/
		...
		if (Err == 0) {								/* Close all files that are open on that drive	*/
			...
			else if (FF_ERR_NONE != FF_UnmountPartition(g_SysMnt[Drv].Fsys)) {
				Err = EACCES;
			}
			else if (FF_ERR_NONE != FF_DestroyIOMAN(g_SysMnt[Drv].Fsys)) {
				Err = EACCES;
			}
		}
		...
		if (Err == 0) {								/* The unmounting was successful, then we can	*/
			g_SysMnt[Drv].MntPoint[0] = '\0';		/* make this FS descriptor available. A zero	*/
				media_uninit_FullFAT(Drv);			/* length name for mount point/drive means free	*/
		}											/* De-inint to re-initialize the next time		*/
...
/*																									*/
/* NOTE:																							*/
/*		It does not look like FullFAT's formatting is working properly.  The FullFAT demos and		*/
/*		tests have all instances of FF_FormatPartition() commented out.  For safety, mkfs() is 		*/
/*		always returning an error.  The full code is left here in case it behaves better in a		*/
/*		future release.																				*/
/*																									*/
/*      FullFAT formatting API uses the cluster size.  From that info, it is not possible to		*/
...
/*		the resulting format is determined by FullFAT.												*/
/*																									*/
/* IMPLEMENTATION:																					*/
/*																									*/
/* ------------------------------------------------------------------------------------------------ */
...
int64_t  DevSize;									/* Device size to determine cluster size		*/
int      Drv;										/* Physical device / drive						*/
int      Err;										/* Error tracking								*/
FF_ERROR Fres;										/* Result of FullFAT calls						*/

	Err     = ENXIO;								/* See NOTE: above, abort ASAP					*/
	Cluster = 0;
	...
	if (Err == 0) {
		...
		if (Err == 0) {
			g_SysMnt[Drv].Fsys = FF_CreateIOMAN(NULL, 2*BlkSize, BlkSize, &Fres);
			if (g_SysMnt[Drv].Fsys == NULL) {
				FF_DestroyIOMAN(g_SysMnt[Drv].Fsys);
				Err = ENXIO;
			}
		...
		if ((Err == 0)
		&&  (Fres == FF_ERR_NONE)
		&&  (FF_ERR_NONE != FF_RegisterBlkDevice(g_SysMnt[Drv].Fsys, 512,
		                                        (FF_WRITE_BLOCKS) media_write_FullFAT,
		                                        (FF_READ_BLOCKS)  media_read_FullFAT,
			                                    (void *)(intptr_t)Drv))) {
			Err = EACCES;
		...
		&&  (Fres != FF_ERR_NONE)) {
			...
			DevSize = media_size_FullFAT(Drv);
			if (DevSize < 64LL*1024LL*1024LL) {
				Cluster = 512;
			}
			else if (DevSize < 128*1024LL*1024LL) {
				Cluster = 1024;
			}
			else if (DevSize < 256*1024LL*1024LL) {
				Cluster = 2048;
			}
			else if (DevSize < 8LL*1024LL*1024LL*1024LL) {
				Cluster = 4096;
			}
			else if (DevSize < 16LL*1024LL*1024LL*1024LL) {
				Cluster = 8192;
			}
			else {
				Cluster = 16384;
			...
			if (FF_ERR_NONE != FF_FormatPartition(g_SysMnt[Drv].Fsys, Drv, Cluster)) {
				media_init_FullFAT(Drv);			/* Automatic retry 								*/
				if (FF_ERR_NONE != FF_FormatPartition(g_SysMnt[Drv].Fsys, Drv, Cluster)) {
					Err = ENXIO;
				}
			}
			...
			FF_FlushCache(g_SysMnt[Drv].Fsys);
			FF_UnmountPartition(g_SysMnt[Drv].Fsys);
			FF_DestroyIOMAN(g_SysMnt[Drv].Fsys);	/* mkfs does not perform auto-mounting			*/
		}
	}
...
/*       a data type used by FullFAT; so there is a conflict. As FullFAT should remain untouched,	*/
/*       it was necessary to use a data type with a difference name, therefore the use of DIR_t.	*/
/*																									*/
...
char     Lpath[SYS_CALL_MAX_PATH+1];				/* Logical full path							*/
	...
	if (Err == 0) {
		...
		if (dd >= (int)(sizeof(g_SysDir)/sizeof(g_SysDir[0]))) {	/* All entries are used			*/
			...
			if (3 != strlen(Lpath)) {				/* FullFAT needs directories to end with '/'	*/
				strcat(&Lpath[0], "/");				/* Don't add '/' on root level					*/
			}

			Fres =  FF_FindFirst(g_SysMnt[Drv].Fsys, &g_SysDir[dd].Ddesc, &Lpath[2]);
			if ((Fres != FF_ERR_NONE)				/* There is a special case when at root level	*/
			&&  ((FF_GETERROR(Fres) != FF_ERR_DIR_END_OF_DIR)	/* and it's empty					*/
			   || (0 != strcmp(&Lpath[2], "/")))) {
				if (IsMnt != 0) {
...
/*		Use FullFAT f_readdir() and if successful, extract all relevant information from it.		*/
/*																									*/
/* ------------------------------------------------------------------------------------------------ */
...
{
int        Err;										/* Error tracking								*/
FF_DIRENT *FileInfo;								/* Dir information reported by FullFAT			*/
FF_ERROR   Fres;									/* Result of FullFAT calls						*/
int        ii;										/* General purpose								*/
struct dirent *Ret;									/* Return value									*/
	...
	if ((Err == 0)									/* The directory is open, use FullFAT directly	*/
	&&  (Ret ==  (struct dirent *)NULL)) {			/* When != NULL, is reporting a mount point		*/
		if (dirp->Drv < 0) {						/* Is a partial mount point path				*/
			FileInfo->FileName[0] = '\0';			/* We are done reading the dir					*/
		}
		else {
			Fres = FF_FindNext(g_SysMnt[dirp->Drv].Fsys, FileInfo);
			if (Fres != FF_ERR_NONE) {				/* Read error, but don't want to set errno when	*/
				Err = ENOENT;						/* EOD has been reached, want to return NULL	*/
				if ((Fres & 0xFFF) == FF_ERR_DIR_END_OF_DIR) {
					Err = 0;
	...
	&&  (Ret ==  (struct dirent *)NULL)				/* When != NULL, is reporting a mount point		*/
	&&  (FileInfo->FileName[0] != '\0')) {			/* There is a valid file name, extract info		*/
		Ret            = &(dirp->Entry);
		strncpy(Ret->d_name, &FileInfo->FileName[0], SYS_CALL_MAX_PATH);
		Ret->d_name[SYS_CALL_MAX_PATH] = '\0';
		Ret->d_namlen  = strlen(Ret->d_name);
		Ret->d_seekoff = dirp->Offset;
		Ret->d_type    = (FileInfo->Attrib & FF_FAT_ATTR_DIR)
		               ? DT_DIR
		               : DT_REG;
	}
...
void SC_SEEKDIR_(P_NM, DIR_t *dirp, long loc)
{
	...
	else {											/* Directory is open, use FullFAT directly		*/
...
{
	return(media_blksize_FullFAT(Drv));
}
...
{
	return(media_sectsize_FullFAT(Drv));
}
...
int Ret;
	...
	&&  (g_SysFile[fh].IsOpen != 0)) {
		if (FF_ERR_NONE == FF_FlushCache(g_SysFile[fh].Fdesc->pIoman)) {
			Ret = 0;
		}
	}
...
long Ret;
	...
	||  (fh < (int)(sizeof(g_SysFile)/sizeof(g_SysFile[0])))
	&&  (g_SysFile[fh].IsOpen != 0)) {
		Ret = (long)g_SysFile[fh].Fdesc->Filesize;
	}
	...
	if (fd < 0) {									/* fd-ve is flush all							*/
		for (fd=0 ; fd<(int)(sizeof(g_SysFile)/sizeof(g_SysFile[0])) ; fd++) {
			if (0 == FIL_MTXlock(fd)) {				/* In case deadlock detection is used			*/
				if (g_SysFile[fd].IsOpen != 0) {
					FF_FlushCache(g_SysFile[fd].Fdesc->pIoman);
				}
				FIL_MTXunlock(fd);
			}
		}
		...
		if (fd >= (int)(sizeof(g_SysFile)/sizeof(g_SysFile[0]))) {
			...
			if (0 == FIL_MTXlock(fd)) {				/* In case deadlock detection is used			*/
				...
				else if (FF_ERR_NONE != FF_FlushCache(g_SysFile[fd].Fdesc->pIoman)) {
					Ret = -1;
				}
				FIL_MTXunlock(fd);
			}
