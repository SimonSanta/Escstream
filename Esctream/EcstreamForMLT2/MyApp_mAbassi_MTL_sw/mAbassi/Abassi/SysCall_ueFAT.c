/* ------------------------------------------------------------------------------------------------ */
/* FILE :		SysCall_ueFat.c																		*/
/*																									*/
/* CONTENTS :																						*/
/*				System call layer with underlying ultra-embedded FAT								*/
/*																									*/
/*																									*/
...
/* ------------------------------------------------------------------------------------------------ */
/* Limitations / non-standard things due to ueFAT													*/
/*																									*/
/* ueFAT can only unlink a regular file. It has no provisions to unlink a directory.				*/
/* If the path/name to unlink is determined to be a directory, unlink() is set to return an error.	*/
/*																									*/
/* ueFAT does not provide a direct access to modify the file attribute bit. So it is not			*/
/* straight-forward change a file attribute, meaning support for chmod() cannot be provided.		*/
/* chmod() will always return an error because of that.												*/
/* Note that the RO information is pulled from ueFAT internal and is valid, but changing between	*/
/* RO and RW is not available.																		*/
/*																									*/
/* ueFAT does not allow an open file to be opened again, even in RO access.							*/
/* Because of that, dup() is a bit useless but is still provided in case ueFAT removes the opening	*/
...
/* ------------------------------------------------------------------------------------------------	*/
  ...
  #define SYS_CALL_N_FILE 	FATFS_MAX_OPEN_FILES	/* Excluding stdin, stdout & stderr				*/
#endif												/* Use ueFAT setup as default					*/
  ...
  #define SYS_CALL_N_DRV		1					/* 1 as ueFAT only handles a single drive		*/
#endif
...
/* ------------------------------------------------------------------------------------------------ */
  ...
  #define SYS_CALL_MULTI_UEFAT		0				/* By default, single file system stack			*/
#endif

#if ((SYS_CALL_MULTI_UEFAT) != 0)					/* When more than 1 FS stack is used			*/
  #define P_NM _ueFAT								/* Each ones of the function in here are		*/
#else												/* renamed with the post-fix "_ueFAT".  The		*/
...
typedef struct {
	struct fatfs *Fsys;								/* ueFAT file system descriptor					*/
...
typedef struct {									/* Our own file descriptors						*/
	...
	FL_FILE *Fdesc;									/* ueFAT file descriptor						*/
...
typedef struct SysDir {								/* Our own directory descriptors				*/
    ...
    FL_DIR Ddesc;									/* ueFAT directory descriptor					*/
	FL_DIR Dfirst;									/* First entry in the open directory (for seek)	*/
...
extern void fl_init_locks(void);
extern void fl_lock(void);
extern void fl_unlock(void);
...
#define SX_IARFLUSH_(P2,x)						IARflush##P2(x)
  ...
  #define REENT_ERRNO_							(errno)
	...
	int fstat_ueFAT(int fd, struct stat *pstat);
	int fstatfs_ueFAT(int fs, struct statfs *buf);
	int stat_ueFAT(const char *path, struct stat *pstat);
	int unlink_ueFAT(const char *path);
  #endif
  ...
  #if ((SYS_CALL_MULTI_UEFAT) != 0)
	int _fstat_r_ueFAT(struct _reent *reent, int fd, struct stat *pstat);
	int _fstatfs_r_ueFAT(struct _reent *reent, int fs, struct statfs *buf);
	int _stat_r_ueFAT(struct _reent *reent, const char *path, struct stat *pstat);
	int _unlink_r_ueFAT(struct _reent *reent, const char *path);
  #endif
...
#if ((SYS_CALL_MULTI_UEFAT) != 0)
  DIR_t         *opendir_ueFAT(const char *path);
  struct dirent *readdir_ueFAT(DIR_t *dirp);
  int            closedir_ueFAT(DIR_t *dirp);
#endif

#define MY_FAT_NAME	"ueFAT"
...
/* ------------------------------------------------------------------------------------------------ */
/* Local defintions as no Media_ueFAT.h available													*/
...
extern int         media_init_ueFAT     (int Drv);
extern int         media_read_ueFAT     (uint32 sector, uint8 *buff, uint32 count);
extern int64_t     media_size_ueFAT     (int Drv);
extern int         media_uninit_ueFAT   (int Drv);
extern int         media_write_ueFAT    (uint32 sector, uint8 *buff, uint32 count);
...
char   *FatFlags;									/* Remapped flags to the ueFAT tokens			*/
...
struct stat pstat;
	...
	Err      = 0;
	FatFlags = (char *)NULL;						/* To remove compiler warning					*/
	...
	if (Access == O_WRONLY) {						/* These 3 O_??? are not ORed bit definitions	*/
		Omode    = 0333;							/* Need to do one on one comparisons			*/
		...
		Omode    = 0555;							/* if caller forgot to set the desired access	*/
		FatFlags = "r";
	}
	...
	if (Err == 0) {									/* A global lock is more R/T efficient than		*/
		if (0 != MTXlock(g_GlbMtx, -1)) {			/* per file lock								*/
			Err = ENOLCK;							/* In case deadlock detection is enable			*/
		}
	}

	if (Err == 0) {
		if ((Drv >= 0)
		&&  ((flags & O_CREAT) != 0)) {				/* With O_CREAT|O_EXCL, the file to open can't	*/
			if (0 == SC_STAT_(P_NM, reent, Rpath, &pstat)) {	/* exist. ueFAT does not have such	*/
				if ((flags & O_EXCL) != 0) {		/* a flag										*/

					Err = EEXIST;
				}
				else if ((flags & O_TRUNC) != 0) {	/* ueFAT does not zero the size of an existing	*/
					unlink(Rpath);					/* although it does have such a flag			*/
				}
			}
		}
		...
		if (Err == 0) {
			...
			{										/* Opening a regular file						*/
				...
				if (Err == 0) {
					g_SysFile[fd].Fdesc = fl_fopen(&Rpath[2], FatFlags);
					if (g_SysFile[fd].Fdesc == (FL_FILE *)NULL) {
						Err = EACCES;
					}
				}
...
int  Err;											/* Error tracking								*/
	...
	||  (fd >= (int)(sizeof(g_SysFile)/sizeof(g_SysFile[0])))) {
		...
		&&  (g_SysFile[fd].DevType == 0)
			...
			if (0 !=  fl_fflush(g_SysFile[fd].Fdesc)) {
				Err = EACCES;
			}

			fl_fclose(g_SysFile[fd].Fdesc);
  ...
  int DevType = 0;
	...
	||  (fd >= (int)(sizeof(g_SysFile)/sizeof(g_SysFile[0])))) {
		...
		if (Err == 0) {								/* File open and read operation authorized		*/
		  ...
		  #endif
			{
				Ret = fl_fread(vbuf, 1, size, g_SysFile[fd].Fdesc);
				if (Ret < 0) {
					Err = EACCES;					/* When has reached EOF, ueFAT returns error	*/
					if (0 != fl_feof(g_SysFile[fd].Fdesc)) {
						Err = 0;					/* Is the EOF, so not an error as per standard	*/
						Ret = 0;
					}
				}
			}
  ...
  int DevType = 0;
	...
	||  (fd >= (int)(sizeof(g_SysFile)/sizeof(g_SysFile[0])))) {
		...
		if (Err == 0) {
		  ...
		  #endif
			{
				Ret = fl_fwrite(vbuf, 1, len, g_SysFile[fd].Fdesc);
				if (Ret < 0) {
					Err = EACCES;
				}
				if ((Err == 0)
				&&  (g_SysFile[fd].Sync != 0)) {	/* If open with sync, flush after each write	*/
					fl_fflush(g_SysFile[fd].Fdesc);
				}
			}
		}
...
off_t OffRes;										/* Resulting offset								*/
	...
	||  (fd >= (int)(sizeof(g_SysFile)/sizeof(g_SysFile[0])))) {
		...
		if (Err == 0) {
			fl_fflush(g_SysFile[fd].Fdesc);			/* Must flush, otherwise ueFAT seek fails		*/
			if (0 != fl_fseek(g_SysFile[fd].Fdesc, (long)offset, whence)) {
				Err = EACCES;
			}
		}
...
/* NOTE:																							*/
/*		ueFAT can only unlink a regular file. It has no provisions to unlink a directory.			*/
/*		If the path/name to unlink is a directory, unlink() will returns an error.					*/
/*																									*/
/* IMPLEMENTATION:																					*/
/*																									*/
/* ------------------------------------------------------------------------------------------------ */
...
char Rpath[SYS_CALL_MAX_PATH+1];
	...
	if ((Err == 0)									/* Simple: direct use of the ueFAT function		*/
	&& (0 == fl_createdirectory(&Rpath[2]))) {
		Err = EACCES;
	}
...
/*																									*/
/* IMPLEMENTATION:																					*/
/*		Uses _stat_r() as ueFAT delivers stats using filenames										*/
/*																									*/
/* ------------------------------------------------------------------------------------------------ */
...
int     Err;										/* Error tracking								*/
int32_t DateTime;
char    FileName[SYS_CALL_MAX_PATH+1];
int     ii;											/* General purpose								*/
int     IsMnt;										/* Is is a mount point full or or partial path	*/
DIR_t  *MyDir;
char   *Ptr;

struct fat_dir_entry Info;
	   ...
	   int Len;										/* Length of path (without the file name		*/
	  ...
	  #endif
		{
			...
			if (IsMnt != 0) {						/* Yes it is a mount point						*/
				...
				Drv = RealPath(&Rpath[0], path, NULL);
				Ptr = strrchr(&Rpath[0], '/');		/* Split into dir & filename (need to read dir	*/
				strcpy(&FileName[0], Ptr+1);
				if (Ptr == &Rpath[0]) {
					Ptr++;
				}
				*Ptr= '\0';
				MyDir = SC_OPENDIR_(P_NM, &Rpath[0]);
				if (MyDir == (DIR_t *)NULL) {
					Err = REENT_ERRNO;
				}

				if (Err == 0) {						/* Read the dir until we find the file			*/
					while(NULL != (DirFile = SC_READDIR_(P_NM, MyDir))) {
						if (0 == strcasecmp(&DirFile->d_name[0], FileName)) {
							break;
						}
					}
					if (DirFile == (struct dirent *)NULL) {
						Err = ENOENT;
					}
				}

				if (Err == 0) {
					fatfs_get_file_entry(g_SysMnt[Drv].Fsys, MyDir->Ddesc.cluster, FileName, &Info);

					DateTime     =  (((int32_t)Info.LstAccDate[0]) & 0xFF)
					             + ((((int32_t)Info.LstAccDate[1]) & 0xFF) <<  8);
					Time.tm_year = (0x7F&(DateTime>>9)) + 80;	/* FAT32 0-time: 1980.01.01 00:00:00*/
					Time.tm_mon  =  0x0F&(DateTime>>5);
					Time.tm_mday =  0x1F&(DateTime);
					...
					pstat->st_size  = (off_t)Info.FileSize;	/* Size of the file							*/

					DateTime     =  (((int32_t)Info.WrtDate[0]) & 0xFF)
					             + ((((int32_t)Info.WrtDate[1]) & 0xFF) <<  8);
					Time.tm_year = (0x7F&(DateTime>>9)) + 80;	/* FAT32 zero-time: 1980.01.01 00:00:00	*/
					Time.tm_mon  =  0x0F&(DateTime>>5);
					Time.tm_mday =  0x1F&(DateTime);

					DateTime     =  (((int32_t)Info.WrtTime[0]) & 0xFF)
					             + ((((int32_t)Info.WrtTime[1]) & 0xFF) <<  8);
					Time.tm_hour =  0x1F&(DateTime>>11);
					Time.tm_min  =  0x3F&(DateTime>>5);
					Time.tm_sec  =  0x3E&(DateTime<<1);
					pstat->st_mtime = mktime(&Time);
					pstat->st_ctime = pstat->st_mtime;
					pstat->st_mode  = (S_IRUSR | S_IRGRP | S_IROTH)	/* ueFAT is always readable			*/
					                | (S_IXUSR | S_IXGRP | S_IXOTH)	/* ueFAT is always executable		*/
					                | (((Info.Attr) & FILE_ATTR_READ_ONLY)
					                  ? 0					/* When read-only, no write privilege		*/
					                  : (S_IWUSR | S_IWGRP | S_IWOTH))
					                | (((Info.Attr) & FILE_ATTR_DIRECTORY)
				    	              ? S_IFDIR				/* Set to directory or regular file			*/
				        	          : S_IFREG);
...
{
char    DrvNmb[3];									/* Drive # string as required by ueFAT			*/
int     Err;										/* Error tracking								*/
	...
	if (Err == 0 ) {
		...
		buf->f_bsize = FAT_SECTOR_SIZE;				/* ueFAT sector size are fixed					*/

		buf->f_blocks = media_size_ueFAT(fs)
		              / FAT_SECTOR_SIZE;
		buf->f_bfree  = 0;							/* ueFAT does not compute or memo free space	*/
...
/*																									*/
/* IMPLEMENTATION:																					*/
/*		ueFAT does not have an API to access a file attributes, nor does the fl_dirent data type	*/
...
/*																									*/
/* NOTE:																							*/
/*		ueFAT is single drive, therefore data must be "0" / "0:"									*/
/*		Only the flag MNT_RDONLY is handled due to the limitations of FAT32							*/
/*																									*/
/* IMPLEMENTATION:																					*/
/*																									*/
/* ------------------------------------------------------------------------------------------------ */
...
char *Ptr;											/* "data" argument is the logical drive #		*/
	...
	if ((Err == 0)
	&&  (g_MountCnt != 0)) {						/* ueFAT can only handle a single drive			*/
		Err = EACCES;
	}
	...
	if (Err == 0) {
		if (g_SysMnt[Drv].MntPoint[0] != '\0') {	/* Is mounted, can only update					*/
			...
			&&  (0 == media_init_ueFAT(Drv))) {
				Err = ENXIO;
			}

			if ((Err == 0)							/* Attach the media access drivers				*/
			&&  (fl_attach_media(media_read_ueFAT, media_write_ueFAT) != FAT_INIT_OK)) {
		        Err = ENXIO; 
			}

			if (Err == 0) {							/* Attach the ueFAT mutex						*/
				fl_init_locks();
				fl_attach_locks(&fl_lock, &fl_unlock);
			}

			if (Err == 0) {							/* Memo the mount point in g_SysMnt				*/
				g_SysMnt[Drv].Fsys  = fl_get_fs();	/* Memo the file system descriptor as need to 	*/
				g_SysMnt[Drv].Flags = flags;
				switch (g_SysMnt[Drv].Fsys->fat_type) {
				case FAT_TYPE_16:
...
int  Len;											/* String length								*/
	...
	if (0 != MTXlock(g_GlbMtx, -1)) {				/* Need global protection for that				*/
		...
		if (Err == 0) {								/* Close all files that are open on that drive	*/
			...
			if (g_SysMnt[Drv].MntPoint[0] == '\0') {/* Drv is the FS descriptor to use				*/
				Err = EINVAL;						/* Must be mounted to be unmounted				*/
			}
		}
		...
		if (Err == 0) {								/* The unmounting was successful, then we can	*/
			...
			media_uninit_ueFAT(Drv);				/* length name for mount point/drive means free	*/
...
int     Err;										/* Error tracking								*/
int     FatType;									/* (auto) FAT16 or FAT32 depends on media size	*/
int64_t Size;										/* Media size									*/
	...
	Drv = ((char *)data)[0]-'0';					/* Invalidate the dummy mount					*/
	if ((Drv < 0 )									/* mounted or not mounted						*/
	&&  (Drv >= (sizeof(g_SysMnt)/sizeof(g_SysMnt[0])))) {
		Err = EBADF;
	}
	...
	if (Err == 0) {
		...
		else {										/* Do dummy mount to make sure media I/F is OK	*/
			if (g_MountCnt != 0) {					
				Err = EACCES;
			}
			else {
				Err = REENT_ERRNO_;
				mount(type, "/0_0_0", 0, (char *)data);
				REENT_ERRNO_ = Err;
				Err = 0;
				g_SysMnt[Drv].MntPoint[0] = '\0';	/* Set to "": will not be mounted after format	*/
			}
		}

		Size = media_size_ueFAT(Drv);				/* Get the size of the media					*/
		if (0 == strcasecmp(type, FS_TYPE_NAME_AUTO)) {
			FatType = (Size < (2LL * 1024LL * 1024LL * 1024LL))
			        ? 16
			        : 32;
		}
		else if (0 == strcasecmp(type, FS_TYPE_NAME_FAT16)) {
			Size    = 4LL * 1024LL * 1024LL * 1024LL;
			FatType = 16;							/* FAT16 is limited to device up to 4G			*/
		}
		...
		if (Err == 0) {
			Size = Size								/* Convert media size to number of sectors		*/
			     / FAT_SECTOR_SIZE;
			if (FatType == 16) {
				if (0 == fatfs_format_fat16(fl_get_fs(), Size, "")) {
					Err = ENOSPC;
				}
			}
			else {
				if (0 == fatfs_format_fat32(fl_get_fs(), Size, "")) {
					Err = ENOSPC;
				}
			}
		}
		if (Err == 0) {								/* Remove the mount point						*/
			unmount("/0_0_0", 0);
		}
	}
...
/*       a data type used by ueFAT; so there is a conflict. As ueFAT should remain untouched, it	*/
...
int     NoMnt;										/* If nothing has been mounted					*/
DIR_t  *Ret;
char    Lpath[SYS_CALL_MAX_PATH+1];					/* Logical full path							*/
	...
	&&  (NoMnt == 0)) {
		if (0 != MTXlock(g_GlbMtx, -1)) {			/* Need global protection: modifying .IsOpen	*/
			Err = ENOLCK;							/* In case deadlock detection enable			*/
		}
	}
	...
	&&  (NoMnt == 0)) {
		...
		if (dd >= (int)(sizeof(g_SysDir)/sizeof(g_SysDir[0]))) {	/* All entries are used			*/
			...
			if (NULL == fl_opendir(&Lpath[2], &g_SysDir[dd].Ddesc)) {
				if (IsMnt != 0) {
...
/*		Use ueFAT fl_readdir() and if successful, extract all relevant information from it.			*/
/*																									*/
/* ------------------------------------------------------------------------------------------------ */
...
{
int       Err;										/* Error tracking								*/
fl_dirent FileInfo;									/* File information reported by ueFAT			*/
int       ii;										/* General purpose								*/
struct  dirent *Ret;								/* Return value									*/
	...
	&&  (Ret ==  (struct dirent *)NULL)) {			/* When != NULL, is reporting a mount point		*/
		if (dirp->Drv < 0) {						/* Is a partial mount point path				*/
			...
			if (0 != fl_readdir(&(dirp->Ddesc), &FileInfo)) {
				Err = ENOENT;						/* Read error, but don't want to set errno when	*/
				if (dirp->Ddesc.offset == 0) {		/* EOD has been reached, want to return NULL	*/
					Err = 0;
	...
	&&  (Ret ==  (struct dirent *)NULL)				/* When != NULL, is reporting a mount point		*/
	&&  (FileInfo.filename[0] != '\0')) {			/* Dir reading is OK, grab the useful info		*/
		Ret            = &(dirp->Entry);			/* There is a valid filename, extract info		*/
		strncpy(Ret->d_name, &FileInfo.filename[0], SYS_CALL_MAX_PATH);
		Ret->d_name[SYS_CALL_MAX_PATH] = '\0';
		Ret->d_namlen  = strlen(Ret->d_name);
		Ret->d_seekoff = dirp->Offset;
		Ret->d_type    = (FileInfo.is_dir)
		               ? DT_DIR
		               : DT_REG;
	}
...
void SC_SEEKDIR_(P_NM, DIR_t *dirp, long loc)
{
	...
	else {											/* Directory is open, use ueFAT directly		*/
...
{
	return(media_blksize_ueFAT(Drv));
}
...
{
	return(media_sectsize_ueFAT(Drv));
}
...
int Ret;
	...
	&&  (g_SysFile[fh].IsOpen != 0)) {
		if (0 == fl_fflush(g_SysFile[fh].Fdesc)) {
			Ret = 0;
		}
	}
...
long Ret;
	...
	||  (fh < (int)(sizeof(g_SysFile)/sizeof(g_SysFile[0])))
	&&  (g_SysFile[fh].IsOpen != 0)) {
		Ret = (long)g_SysFile[fh].Fdesc->filelength;
	}
	...
	if (fd < 0) {									/* fd-ve is flush all							*/
		...
		if (fd >= (int)(sizeof(g_SysFile)/sizeof(g_SysFile[0]))) {
			...
			if (0 == FIL_MTXlock(fd)) {				/* In case deadlock detection is used			*/
				...
				else if (0 != fl_fflush(g_SysFile[fd].Fdesc)) {
					Ret = -1;
				}
				FIL_MTXunlock(fd);
			}
